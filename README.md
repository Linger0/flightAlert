# Flight Ticket Price Monitoring Program

By invoking the [Ctrip API](https://github.com/liangen1/-xiechengjipiao_aip), this program monitors flight ticket prices and sends a notification through WeChat when the price falls below the set value (only for weekends).

The WeChat push function is implemented through [ServerChan](https://sc.ftqq.com/3.version).

Please note that this program is for learning purposes only and should not be used for commercial purposes. The project is forked from https://github.com/omegatao/flightAlert.

## How to use

1. Check your Python version >3.6.
2. Clone the project and get into the corresponding directory.
...
5. Configure the `config.json` file. Detailed configuration instructions are provided following.
...
The configuration for the `config.json` file is as follows：
-   placeFrom: departure airport code, details can be found in the following section
-   placeTo: arrival airport code, details can be found in the following section
-   flightWay: type of ticket, OneWay for one-way ticket, Roundtrip for round-trip ticket
-   targetPrice: notification threshold for ticket price, set as needed
-   ftqq_SCKEY: token for ServerChan, obtained as instructed on the https://sc.ftqq.com/3.version.
...
```json
{
    ...
    "targetPrice": 1000,
    "priceStep": 20,
    "ftqq_SCKEY": ["SCT193323TDdNA98H44LrV4lxv9JMrOxxx", "SCT192982Tl3xYyhPlK5kx31D0S8n4Kxxx"]
}
